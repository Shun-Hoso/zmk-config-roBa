...
F + G を同時押し → 定義元へジャンプ
```
roBa が内部的に「右クリック → F12」を送るため、左下の Cmd キーを押す必要はありません。
...
| **SCROLL** (5) | Default/MOUSE 中に **V+B** 同時押し（トグル）／MOUSE 中に **B タップ**（トグル）／トラックボール操作で自動 | トラックボールがスクロールに変化 |
...
② F + G を同時押しする
③ 右クリックメニューの Go to Definition（F12）が実行され、定義元へジャンプ
```

左下の Cmd キーを押す操作ではありません。`F + G` を押すと、roBa が内部的に「右クリック → F12」を送ります。
...
- **SCROLL レイヤー（Layer 5）**が ON のとき、トラックボールでスクロール（横スクロールも可能）
  - ON/OFF: Default/MOUSE 中に **V+B** 同時押し、または MOUSE レイヤー中に **B タップ**（トグル）
  - トラックボールを動かすと MOUSE レイヤー経由で SCROLL に入ることもあります
...
| **V + B** | SCROLL レイヤー ON/OFF | Default/MOUSE 中のみ |
