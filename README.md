...
|------|--------|----------------|----------|
| A の位置 | `a` | `Ctrl`（左） | `Ctrl+L`（URL欄）、`Ctrl+P`（コマンドパレット）など**右手キー**との組合せ |
| ' の位置 | `'` | `Ctrl`（右） | `Ctrl+C/V/X/Z/A` など**左手キー**との組合せ |

**重要: Ctrl は反対の手のキーと組み合わせて使います**（ポジショナル hold-tap）
- Ctrl + 左手キー（C/V/X/Z/A/S/W 等）→ **' を長押し**
- Ctrl + 右手キー（L/P/K/N 等）→ **A を長押し**
...
| Ctrl+Z（取消） | **' 長押し + Z** |
| Ctrl+A（全選択/行頭） | **' 長押し + A** |
| Ctrl+P（コマンドパレット） | **A 長押し + P** |
| Ctrl+L（URL欄/クリア） | **A 長押し + L** |

Shift は **左下段の Shift キー**（左小指の最下段）を使います。

#### キーリピート
A, ', Space, Enter は長押しするとモディファイア/レイヤーが発動するため、**通常の長押しではキーリピートしません**。
...
#### ターミナル操作
```
Ctrl+C (停止)       → ' 長押し + C（クロスハンド）
Ctrl+V (ペースト)   → ' 長押し + V（クロスハンド）
Ctrl+A (行頭移動)   → ' 長押し + A（クロスハンド）
Ctrl+Z (サスペンド)  → ' 長押し + Z（クロスハンド）
Ctrl+E (行末移動)   → A 長押し + E
Ctrl+L (クリア)      → A 長押し + L
Ctrl+R (履歴検索)    → A 長押し + R
Ctrl+P (前の履歴)    → A 長押し + P
