...
└─────┴─────┴─────┴─────┴─────┘│/NAV│└───┘─────┴─────┘     │     └─────┘
                                └────┘
小指側 ──────────────────→ 中央（割れ目）
[Shift] [Alt] [Cmd] 同時押し → BT操作（次の1キーだけ）
...
roBa を手に取ったら、まずこの 8 つだけ覚えてください。
...
Enter を押している間だけ FUNC レイヤーになり、左手の Z〜B がシステム操作に変わります。

#### 8. VSCode / Cursor のコードジャンプ
```
...
|------|--------|----------------|----------|
| ; の位置 | `;` | `Cmd` | `Cmd+C/V/X/T/W/S` など GUI ショートカット |
...
- **GUI のコピー・保存など**: **; 長押し + 文字** で Cmd 系（例: Cmd+C、Cmd+V、Cmd+T）。左手親指下段の Cmd キーも従来通り使えます
- **' 長押し** は Cmd ではなく Ctrl です
...
```
     ...
     V+B トグル       トラックボール動かす   Shift+Alt+Cmd 同時押し
     ┌─────┴─────┐ ┌─────┴─────┐ ┌─────┴─────┐
...
| **MOUSE** (4) | トラックボールを動かすと **自動遷移** | F/S/G で左・右・Shiftクリック |
| **SCROLL** (5) | Default/MOUSE/SCROLL 中に **V+B** 同時押し（トグル） | トラックボールがスクロールに変化 |
| **BT** (6) | **Shift + Alt + Cmd** を同時押し → 次の1キー | Bluetooth 接続管理 |

**重要**: NAV / NUM / FUNC のレイヤーキーは **押している間だけ** 有効です（NUM/SCROLL のトグル、MOUSE の自動遷移は例外）。BT は同時押し後の**次の1キーだけ**有効です。英数キーは英数切替専用です。
...
| **ブラウザ / Notion の戻る** | **NAV + ,（Cmd+[）** |
| **ブラウザ / Notion の進む** | **NAV + .（Cmd+]）** |
...
| `` ` `` | Space + ;（N の左隣） | JS テンプレートリテラル、マークダウン |
| `~` | Space 押しながら I | ホームディレクトリ `~/` |
...
| **前の検索結果へ戻る** | **Enter 押しながら N（⌘⇧G）** |
| **次の検索結果へ進む** | **Enter 押しながら M（⌘G）** |
...
ブラウザ内検索は、`Enter + F` で検索欄を開き、文字列を入力してから `Enter + M` で次、`Enter + N` で前へ移動します。検索結果を送るために単独の F は押しません。
...
MOUSE レイヤーはトラックボールの最終移動から最大5秒間有効ですが、S/F/G以外のキーを押すと即座に解除され、そのキーを通常入力します。
また、文字入力から200ms以内のトラックボール移動ではMOUSEレイヤーを有効にせず、タイピング中の微動による誤操作を抑えます。

#### クリック

通常は右手でトラックボールを動かし、左手のキーを単独で押します。英数キーを押す必要はありません。
...
| 右クリック | S |
| Shift+クリック（範囲選択） | G |
| ドラッグ | Fをホールドし、トラックボールを動かす |
| コードジャンプ | F + G（英数は押さない） |

クリック後はダブルクリックのため250msだけMOUSEレイヤーを維持します。別の場所をクリックするときは、トラックボールを動かすたびに5秒の有効時間が更新されます。

中クリックは割り当てていません。リンクを新しいタブで開く場合は、必要に応じて `F + G` の Cmd+左クリックを使います。

トラックボール移動直後、クリックせずに最初の文字として `f` / `s` / `g` を入力すると、それぞれクリック操作になります。文字として確実に入力したい場合は、Shiftを一度タップして離してから入力してください。Shift単体では文字を送らず、MOUSEレイヤーだけを解除します。
...
コードジャンプは通常クリックと異なり、英数キーを押さずに `F + G` を同時押しします。
...
④ G で Shift+クリック（範囲選択完了）
⑤ Cmd+C でコピー
```

**ダブルクリックで単語選択 → Shift+クリックで拡張** も有効です：
```
① F を素早く2回タップ（ダブルクリック → 単語選択）
② トラックボールで選択を広げたい位置へ
③ G で Shift+クリック（選択範囲を拡張）
```
...
```
① トラックボールを動かしてMOUSEレイヤーへ入る
② Fを押しっぱなしにして左クリックを保持する
③ 右手でトラックボールを動かす
④ Fを離してドロップする
```
...
**切替**: タイピングを約0.3秒止めてから **Shift + Alt + Cmd** を同時押しし、1秒以内に操作キーを押す

| 操作 | キー |
|------|------|
| 私用PC Bluetooth（Profile 0 + BLE 出力） | Y |
| 業務用PC Bluetooth（Profile 1 + BLE 出力） | U |
| 予備端末 Bluetooth（Profile 2〜4 + BLE 出力） | I / O / P |
| BLE 出力へ切替 | H |
| USB 出力へ切替 | J |
| USB / BLE 出力をトグル | K |
| ファームウェア書き込みモード | ; の位置（bootloader） |

`Y / U / I / O / P` は Bluetooth プロファイルを選ぶだけでなく、同時に `OUT_BLE` も実行します。
日常用キーマップには `BT_CLR` / `BT_CLR_ALL` を割り当てません。USB 接続で使ったあとに Bluetooth へ戻すときも、Profile キーを押してください。

#### 初回ペアリング

**私用PCでBluetooth接続する**:
1. roBa の電源を入れる
2. **Shift + Alt + Cmd** を同時に押し、続けて **Y** を押す
3. Profile 0 を選択し、出力先を BLE に切り替える
4. 初回のみ私用PC側の Bluetooth 設定から roBa をペアリングする

**業務用PCをProfile 1へ登録する**:
1. 業務用PCの Bluetooth 設定を開く
2. roBa で **Shift + Alt + Cmd** を同時に押し、続けて **U** を押す
3. Profile 1 と BLE 出力へ切り替わるまで数秒待つ
4. 業務用PCの Bluetooth 設定に表示された roBa を選び、ペアリングする
5. **Shift + Alt + Cmd → Y** で私用PC、**Shift + Alt + Cmd → U** で業務用PCへ接続できることを確認する

業務用PCをペアリングするときは、先に必ず **Shift + Alt + Cmd → U** を押してください。Profile 0 は私用PC用として維持します。
すでに業務用PCとペアリング済みで、**Shift + Alt + Cmd → U** で接続できる場合は再ペアリング不要です。

#### BluetoothでPCを切り替える

**私用PCから業務用PCへ切り替える**:
1. **Shift + Alt + Cmd → U** を押す
2. 数秒待つと、Profile 1 の業務用PCへ接続します

**業務用PCから私用PCへ切り替える**:
1. **Shift + Alt + Cmd → Y** を押す
2. 数秒待つと、Profile 0 の私用PCへ接続します

両方のPCで roBa が「接続済み」と表示される場合がありますが、キー入力は選択中のプロファイル側へ送られます。
通常はPC側のBluetoothをオフにしたり、roBaの登録を解除したりする必要はありません。

#### USB接続へ切り替える

**業務用PCで有線USB接続する**:
1. roBa を業務用PCに USB 接続する
2. 必要なら **Shift + Alt + Cmd → J** を押す
3. 出力先を USB に切り替える

USB利用後にBluetoothへ戻す場合は、私用PCなら **Shift + Alt + Cmd → Y**、業務用PCなら **Shift + Alt + Cmd → U** を押します。

**注意**:
- **Shift + Alt + Cmd → Y** は私用PC専用、**Shift + Alt + Cmd → U** は業務用PC専用として使い、同じプロファイルへ2台をペアリングしないでください。
- ペアリング情報を消去する必要がある場合は、`build.yaml` に含まれる `settings_reset` ファームウェアを左右両方へ書き込んでから、通常ファームウェアを書き戻します。
- **Shift + Alt + Cmd → ;** の bootloader はファームウェア書き込み用で、Bluetooth 切替とは別です。
- Bluetooth 接続が不安定なときは、まず **Shift + Alt + Cmd → Y** または **Shift + Alt + Cmd → U** でプロファイル選択と `OUT_BLE` 切替を明示してください。
- 出力先だけを緊急で切り替えたいときは **Shift + Alt + Cmd → K**（`OUT_TOG`）を使えます。
...
`template ${}`     → ` は Space+;、${} は Shift+4 と Space+,/.
```
...
#### Week 4: 応用
- [ ] トラックボール移動後、F/S/G でクリック、右クリック、範囲選択
- [ ] エンコーダでスクロール
