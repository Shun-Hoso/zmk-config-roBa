...
**重要**: NAV / NUM / FUNC / BT のレイヤーキーは **押している間だけ** 有効です（NUM/SCROLL のトグル、MOUSE の自動遷移は例外）。指を離すと Default に戻ります。
...
5. **英数 + Y** で私用PC、**英数 + U** で業務用PCへ接続できることを確認する

業務用PCをペアリングするときは、先に必ず **英数 + U** を押してください。Profile 0 は私用PC用として維持します。
すでに業務用PCとペアリング済みで、**英数 + U** で接続できる場合は再ペアリング不要です。
...
USB利用後にBluetoothへ戻す場合は、私用PCなら **英数 + Y**、業務用PCなら **英数 + U** を押します。

**注意**:
- 通常運用では `BT_CLR` / `BT_CLR_ALL` は使いません。接続情報を消す操作なので、復旧時だけ使います。
- **英数 + Y** は私用PC専用、**英数 + U** は業務用PC専用として使い、同じプロファイルへ2台をペアリングしないでください。
- Profile 1 のペアリングだけをやり直す場合は、先に **英数 + U** でProfile 1を選び、業務用PC側でroBaの登録を解除してから **英数 + /**（`BT_CLR`）を使います。`BT_CLR_ALL` は使いません。
