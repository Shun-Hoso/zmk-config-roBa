...
roBa を手に取ったら、まずこの 6 つだけ覚えてください。
...
| **SCROLL** (5) | **V+B** 同時押し（トグル）／MOUSE 中に **B タップ**（トグル）／トラックボール操作で自動 | トラックボールがスクロールに変化 |
...
- **SCROLL レイヤー（Layer 5）**が ON のとき、トラックボールでスクロール（横スクロールも可能）
  - ON/OFF: **V+B** 同時押し（どのレイヤーでもトグル）、または MOUSE レイヤー中に **B タップ**（トグル）
...
- [ ] FUNC レイヤーで F12（DevTools）と音量・輝度（Z〜B）を使ってみる
