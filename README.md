...
| **DATE_NUM** (7) | **Space + Enter** 同時押し（トグル） | 右手で日付・数字、左手上段で演算子 |

**重要**: NAV / NUM / FUNC / BT のレイヤーキーは **押している間だけ** 有効です（NUM/SCROLL/DATE_NUM のトグル、MOUSE の自動遷移は例外）。指を離すと Default に戻ります。
...
### 🔢 Layer 7: DATE_NUM（日付・数字）

**切替**: **Space + Enter** を同時押し（トグル）

日付、時刻、チケット番号など、数字を続けて入力するときの右手用レイヤーです。
既存の NUM レイヤーはそのまま残しているため、プログラミング記号は従来通り Space 長押しで使えます。
...
└─────┴─────┴─────┴─────┴─────┘ ├─────┼─────┼─────┼─────┼─────┤
                                ...
                                │  0  │  1  │  2  │  3  │  .  │
                                └─────┴─────┴─────┴─────┴─────┘
左親指の英数/Space 位置 → DATE_NUM 解除
BS / Enter / Del はそのまま使えます
```

右手だけで日付・時刻を打ちつつ、計算式やバージョン番号で必要な `+ * =` は左手上段で単発入力できます。
...
| やりたいこと | 操作 |
|-------------|------|
| DATE_NUM に入る | Space + Enter 同時押し |
| DATE_NUM を解除 | Space + Enter 同時押し、または英数/Space 位置をタップ |
| `2026-07-08` のような日付 | 右手の `0〜9` と `-` |
| `12:34` のような時刻 | 右手の `0〜9` と `:` |
| `1+2*3=7` のような式 | 右手の数字 + 左手上段の `+ * =` |
...
| 業務用PC Bluetooth（Profile 1 + BLE 出力、将来用） | U |
...
USB 接続で使ったあとに Bluetooth へ戻すときも、通常は `BT_CLR` / `BT_CLR_ALL` ではなく Profile キーを押してください。
...
**業務用PCでBluetooth接続する（将来用）**:
1. **英数 + U** を押す
2. Profile 1 を選択し、出力先を BLE に切り替える
3. 初回のみ業務用PC側の Bluetooth 設定から roBa をペアリングする

**業務用PCの有線利用から私用PCへ戻す**:
1. 業務用PCから USB ケーブルを抜く
2. **英数 + Y** を押す
3. Profile 0 を選択し、出力先を BLE に切り替える
4. 数秒待つと私用PCの Bluetooth 接続へ戻ります

**注意**:
- 通常運用では `BT_CLR` / `BT_CLR_ALL` は使いません。接続情報を消す操作なので、復旧時だけ使います。
- **英数 + ;** の bootloader はファームウェア書き込み用で、Bluetooth 切替とは別です。
- Bluetooth 接続が不安定なときは、まず **英数 + Y** または **英数 + U** でプロファイル選択と `OUT_BLE` 切替を明示してください。
- 出力先だけを緊急で切り替えたいときは **英数 + K**（`OUT_TOG`）を使えます。
...
| **Space + Enter** | DATE_NUM レイヤー ON/OFF | 日付・数字入力用 |
...
- [ ] DATE_NUM レイヤー（Space+Enter）で日付や数字列を打つ練習
