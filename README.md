...
| **MOUSE** (4) | トラックボールを動かすと **自動遷移** | トラックボール操作の内部制御（文字キーは通常入力） |
| **SCROLL** (5) | Default/MOUSE/SCROLL 中に **V+B** 同時押し（トグル） | トラックボールがスクロールに変化 |
| **BT** (6) | 「英数」（Spaceの左隣）を **押しっぱなし** | Bluetooth 接続管理、明示クリック |
...
MOUSE レイヤー中も `S/D/F/J/K/L` は通常の文字として入力され、単独の文字キーがクリックへ変わることはありません。

#### クリック（英数キーを押しっぱなし）

クリックは MOUSE レイヤーの時間に依存しない明示操作へ統一しています。右手でトラックボールを操作し、左親指で英数キーを押しながら左手のキーを使います。

| 操作 | キー |
|------|------|
| 左クリック | 英数 + F |
| ダブルクリック | 英数を押したまま F を2回タップ |
| 右クリック | 英数 + S |
| Shift+クリック（範囲選択） | 英数 + G |
| ドラッグ | 英数を押したまま F をホールドし、トラックボールを動かす |
| コードジャンプ | F + G（英数は押さない） |

中クリックは割り当てていません。リンクを新しいタブで開く場合は、必要に応じて `F + G` の Cmd+左クリックを使います。

単独の F は常に文字入力です。クリックするときは、トラックボールを動かしてから時間が経っていても、必ず英数キーを押しながら F を押してください。
...
② 英数 + F で左クリック（選択の起点を置く）
③ トラックボールでカーソルを選択終了位置へ
④ 英数 + G で Shift+クリック（範囲選択完了）
⑤ Cmd+C でコピー
```

**ダブルクリックで単語選択 → Shift+クリックで拡張** も有効です：
```
① 英数を押したまま F を素早く2回タップ（ダブルクリック → 単語選択）
② トラックボールで選択を広げたい位置へ
③ 英数 + G で Shift+クリック（選択範囲を拡張）
```
...
クリック操作では英数キーを押しっぱなしにするため、タップ時の入力言語切替は発生しません。
...
#### Week 4: 応用
- [ ] トラックボール + 英数/F/S/G でクリック、右クリック、範囲選択
- [ ] エンコーダでスクロール
