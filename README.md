...
|------|--------|----------------|----------|
| A の位置 | `a` | `Ctrl`（左） | `Ctrl+C`（プロセス停止）、`Ctrl+P`（VSCode コマンドパレット） |
| ' の位置 | `'` | `Ctrl`（右） | `Ctrl+A`（行頭移動）、`Ctrl+E`（行末移動）など左手キーとの組合せ |
| Z の位置 | `z` | `Shift` | 大文字入力、`Shift+矢印`（テキスト選択） |

**コツ**: Ctrl+右手キー → **A を長押し**。Ctrl+左手キー → **' を長押し**。
タップだけなら普通に `a`、`'`、`z` が入力されます。
...
```
Ctrl+C (停止)      → A 長押し + C
Ctrl+A (行頭移動)  → ' 長押し + A（右手 Ctrl が便利）
Ctrl+E (行末移動)  → A 長押し + E
Ctrl+Z (サスペンド) → ' 長押し + Z（右手 Ctrl が便利）
Ctrl+L (クリア)     → A 長押し + L
Ctrl+R (履歴検索)   → A 長押し + R
