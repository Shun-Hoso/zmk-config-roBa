...
#### 6. スクリーンショット（範囲選択）
```
G の右隣（左手内側キー）→ Cmd+Shift+4（範囲選択スクリーンショット）
```
1キーで十字カーソルが起動し、ドラッグで範囲を選択して撮影できます。
...
| **NUM** (2) | 左中親指（Space）を **押しっぱなし** or **ロック** | 数字入力、プログラミング記号 |
...
| 長い数字の入力 | ① Space 押しっぱなし → ② **かなキー位置**をタップ（🔒ロック）→ ③ Space を離す → ④ 自由に数字入力 → ⑤ **かなキー位置 or Space 位置**をタップ（🔓解除） |
